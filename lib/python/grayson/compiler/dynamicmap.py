   ...
   <dax id='${namespace}gid${c}' file='${outputname}' >
    ...
    def execute(self, context={}):
        ...
        tmpOutputDir = os.path.join(
            outputDir, "tmp")  # avoid overwriting replica catalog.
        ...
        main_flow_name = os.path.join(
            outputDir, "%s.dax" % flow.replace(".graphml", ""))
        ...
        if mapType == 'tar':
            ...
            for archiveMember in members:
                ...
                try:
                    ...
                    logger.error(
                        "Encountered IOError %s compiling subdax %s", e.__str__(), output)
                    raise e
                replicaText.append('%s file://%s/%s pool="local"' % (
                ...
                flowContext[
                    'sites'] = "--sites %s" % sites if instanceArgs == "" else ""
        ...
        GraysonUtil.writeFile(
            outputPath=os.path.join(outputDir, main_flow_name),
            data=mainFlowContent)

        logger.debug("dynamic-map: writing output dax: %s" % mainFlowContent)
        replicaText.append('%s file://%s pool="local"' % (
            os.path.basename(main_flow_name), main_flow_name))
        GraysonUtil.writeFile(
            os.path.join(outputDir, "tmp", replicaCatalogName),
            '\n'.join(replicaText))
