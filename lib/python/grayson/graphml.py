...
class Graph (object):
    ...
    def dump(self):
        for node in self.nodes:
            ...
            logger.debug(
                "--node (%s(%s)(%s)) ", node.getId(), node.getLabel(), typeText)
        for edge in self.edges:
            logger.debug(
                "edge: src: %s trg: %s", edge.getSource(), edge.getTarget())
            try:
                source = self.nodeMap[edge.getSource()]
                target = self.nodeMap[edge.getTarget()]
                if not source:
                    targetText = target.getLabel(
                    ) if target else " unknown target"
                    logger.debug(
                        "unable to find node for source id %s with target %s", edge.getSource(), targetText)
                elif not target:
                    sourceText = source.getLabel(
                    ) if source else " unknown source"
                    logger.debug(
                        "unable to find node for target id %s with source %s", edge.getTarget(), sourceText)
                else:
                    logger.debug(
                        "--edge(id=%s)(src=(id=%s):%s, dst=(id=%s):%s), type(%s)",
                        edge.getId(),
                        source.getId(),
                        source.getLabel(),
                        target.getId(),
    ...
    def characters(self, chars):
        ...
        if parent == "Label":
            ...
            if self.isDescription:
                self.nodeType = "{0}{1}".format (self.nodeType, chars)
    ...
    def endElement(self, element):
        ...
        if element == "node":
            ...
            node = self.graph.addNode(
                self.nodeId, self.label, self.nodeType, self.inferredType)
            label = node.getLabel()
            ...
            self.graph.addEdge(
                self.edgeId, self.source, self.target, self.nodeType)
            self.nodeType = None
            self.source = None
            self.target = None
            self.edgeId = None
